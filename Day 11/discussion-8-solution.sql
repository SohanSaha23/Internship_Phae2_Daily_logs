-- 1Write a query to find the addresses (location_id, street_address, city, state_province, country_name) of all the departments.
...
> select concat(e.first_name," ",e.last_name) as employee_name,j.job_title,d.department_id,l.city
    -> from employees e
...
> select d.department_name,count(e.employee_id) as 'number of employees' from departments d
...
> select j.job_title,concat(e.first_name," ",e.last_name),(e.salary-j.min_salary) as difference_salary from employees e
    -> inner join jobs j using(job_id);
...
> select d.department_id,concat(e.first_name," ",e.last_name) as employee_name,e.hire_date,e.salary
    -> from employees e
    -> inner join departments d using(department_id)
